...
if __name__ == "__main__":
    ...
    kns = [15, 15, 15, 15, 10, 10, 10, 5, 5, 5]
    # kns = [5 for i in range(10)]
    for i in range(10):
        ...
        for j in range(10):
            unlabeled_index = scio.loadmat(
                "../data/" + DATASET + "/" + str(
                    LABELED_RATE) + "PER/" + str(i) + ".mat")['unlabeled_index'].reshape((-1,)).astype(np.int)
            Y_train = Y.copy()
            ...
            # acc = accuracy(torch.Tensor(labels[unlabeled_index]), torch.Tensor(Y[unlabeled_index]))
            acc = metrics.classification_report(labels[unlabeled_index], Y[unlabeled_index])
            print("No.", i, ", ", j, DATASET, " C:", C, " ACC:", acc)
            ACCS.append(acc)
        with open("Youtube.txt", "a+", encoding="UTF-8") as f:
            ...
            f.write(
                "Labeled_rate:" + str(LABELED_RATE) + "\tC:" + str(C) + "\tNei:" + str(
                    K_Neighbors) + "\t" + DATASET + "\t" + str(
